<template>
  <div class="s_Scene_Content">
    <div class="s_SceneListBox" id="s_SceneListBox">
      <div class="s_Scene_list" v-for="(item, index) in selectList" :key="index" @click="handleChange(index,$event)" id="countList">
    ...
    <el-collapse class="listBox" style="">
      ...
      <el-collapse-item class="s_SceneInfoBox">
        ...
        <el-form ref="form" :model="sizeForm" label-width="80px" size="mini" class="count_list_Form">
          <el-form-item label="演员姓名：" class="count_list_Performer">
            <el-input v-model="sizeForm.actorName"></el-input>
          </el-form-item>
          <div class="countContractPeriod">
            <el-form-item
...
<script>
  ...
  export default {
    ...
    methods:{
        ...
        //下载
        download(){
            var href = this.host+'/file/download/extract_result_sub2?project_id='+this.$route.query.project_id+'&user_id='+this.user_id
              ...
              if(response.data.ret == '0'){
                  for(var i = 0; i < response.data.data.length ; i++){
                      if(response.data.data[i] != null){
                          ...
                          let obj = {names:response.data.data[i].role_name,id:response.data.data[i].actor_id,sizeForm:j,pages:response.data.data[i].pages,sessions:response.data.data[i].sessions};
                          this.selectList.push(obj)
                      }
                  }
                  setTimeout(()=>{this.handleChange(0)},1000)
              }
          })
      },
      ...
      saveData(){
        if(this.authId==1){
          ...
          form.append("actor_id",this.sizeForm.id);
          if(this.sizeForm.actorName != undefined){
            form.append("actor_name", this.sizeForm.actorName);
          }
          let arr = new Array();
      ...
      handleChange(val,e) {
          ...
          if(e != undefined){
            ...
            if(path.length == 13){
              ...
              $('#countList').addClass("active").siblings().removeClass("active");
          }
          if(this.selectList[val].names != ''){
              this.sizeForm.name = this.selectList[val].names
              this.sizeForm.id = this.selectList[val].id
              axios.get('/play/actor_addon', {
                  params: {
                      actor_id: this.selectList[val].id,
                  }
              }).then((response)=> {
                  if(response.data.ret=='0'){
                      if(response.data.data.actor_name){
                          this.selectList[val].sizeForm.actorName = response.data.data.actor_name
                          this.sizeForm.actorName = response.data.data.actor_name
                      }else{
                          this.sizeForm.actorName = ''
                      }
                      var arr = new Array;
...
<style>
  ...
  .count_list_Form .el-form-item .el-form-item__content .el-input .el-input__inner{
    margin-bottom: 30px;
  }
  .count_list_Form .el-form-item .el-form-item__content .el-input .el-input__inner,
  .count_list_Form .el-form-item .el-form-item__content .el-date-editor{
    height: 34px;
    width: 380px;
  }

  .count_list_Form .el-form-item .el-form-item__content{
    display: flex;
    align-items: center;
    margin-left: 94px!important;
    margin-bottom: 12px;
  }
  .count_list_Form .contractPeriod .el-form-item{
    height: 34px;
    margin-bottom: 12px!important;
  }
  .count_list_Form .el-form-item {
    margin-bottom: 0px;
  }

  .count_list_Form .el-form-item .el-form-item__content .el-date-editor .el-range-input,
  .count_list_Form .el-form-item .el-form-item__content .el-date-editor .el-range-separator,
  .count_list_Form .el-form-item .el-form-item__content .el-date-editor .el-input__icon{
    font-size: 14px;
    line-height: 34px;
  }
  ...
  .count_list_Form .el-form-item .el-form-item__label{
    font-size: 18px;
    font-weight: normal;
...
<style scoped>
  *{
  ...
  #countHot-preview .trBox:first-of-type{
    height: 58px;
    background: rgba(231,231,236,1);
    border-radius: 6px;
    font-size: 16px;
  ...
  #countHot-preview .trBox:first-of-type .thBox{
    line-height: 58px;
    height: 58px;
    padding: 0;
    color: #000;
  ...
  #countHot-preview .trBox .thBox:nth-of-type(1),
  #countHot-preview .trBox .thBox:nth-of-type(5),
  #countHot-preview .trBox .thBox:nth-of-type(4){
    width: 60px;
  }

  #countHot-preview .trBox .thBox:nth-last-of-type(3){
    width: 230px;
  }

  #countHot-preview .trBox .thBox:nth-last-of-type(1),
  #countHot-preview .trBox .thBox:nth-last-of-type(2){
    width: 90px;
    margin: 0 10px;
  }
  ...
  #countHot-preview .trBox .thBox .s_scenTableBtn{
    border: none;
    color: #ED9515;
    padding: 16px 0;
    background: none;
  ...
  #countHot-preview .trBox .thBox .s_scenTableBtn:hover{
    background: none;
  }
